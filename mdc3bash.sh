...
cat history* | grep Commandline | grep -v pastebinit | grep -v cowsay | sed 's/Commandline\: apt-get//g' | sed 's/remove/removed/g' | sed 's/install/installed/g' | sed 's/purge/purged/g' > /rippedhist

cat $(locate bash_history) > /usershistory
...
echo Working on detecting prohibited media...
...
echo $(date): Installing Chkrootkit manually >> /var/log/mikescript.log
